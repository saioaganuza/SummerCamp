    ...
    internal class EjemploOperador1
    {
        ...
        internal void Ejecutar()
        {
            ...
            IEnumerable<Empleado> Empleados = new List<Empleado>()
         {
            ...
            List<Empleado> empleadosDesarrolloSoporte = Empleados.Where(empleado => empleado.Departamento == Departamento.Desarrollo || empleado.Departamento == Departamento.Soporte && empleado.Apellidos.StartsWith("C")).OrderByDescending(empleado => empleado.Nombre).Select(empleado => empleado).ToList();

           List<Empleado> empleadosDesarrolloSoporteConsulta = (from empleado in Empleados where (empleado.Departamento == Departamento.Soporte) && empleado.Apellidos.StartsWith("C") orderby empleado.Nombre descending select empleado).ToList();
            ...
            List<Empleado> listaAntiguosEmpleados = (from empleado in Empleados select empleado).ToList();
                    // no se pueden mezclar estas dos: linQ (1) y métodos (2)
            listaAntiguosEmpleados.AddRange(empleadosNuevos);
        }
    }
}